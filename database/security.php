...
class AuthDB {
  ...
  private PDOStatement $statementDeleteUserAccount;
  ...
  function __construct(private PDO $pdo)
  {
    ...
    $this->statementDeleteUserAccount = $pdo->prepare("DELETE FROM user WHERE id=:id");
  ...
  function login(string $userId): void {
    ...
    $signature = hash_hmac("sha256", $sessionId, "saucisson et jambon cru");
  ...
  function isloggedin(): array | false  {
    ...
    if($sessionid && $signature) {
      $hash = hash_hmac("sha256", $sessionid, "saucisson et jambon cru");
  ...
  function logout(string $sessionId):void 
  {
    ...
    $this->statementDeleteUserAccount->bindValue(":id", $id);
    $this->statementDeleteUserAccount->execute();
  }
